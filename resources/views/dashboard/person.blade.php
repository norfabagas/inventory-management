...
<script>
  $(document).ready(function () {
    ...
    $('#addForm').on('submit', function () {
      ...
      $.ajax({
        url: "{{ url('dashboard/stuff/person-json') }}",
        dataType: 'JSON',
        type: 'POST',
        data: {
          method: '_STORE',
        ...
        success: function (data) {
          ...
          if (data.errors) {
            ...
            $('#addModal').modal('hide');
            toastr.success('New Person Added');
        ...
        buttons: {
          ...
          yes: {
            ...
            callback: function () {
              console.log(id);
              $.ajax({
                url: "{{ url('dashboard/stuff/person-json') }}/" + id,
                type: 'DELETE',
                dataType: 'JSON',
                data: {
                  method: '_DESTROY',
                ...
                success: function (data) {
                  toastr.warning('Person ' + data.msg.name + ' deleted');
                  $('#table').DataTable().draw(false);
                }
              })
            }
